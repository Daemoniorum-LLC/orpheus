...
export class GuitarProParser {
  ...
  async parse(fileBuffer: ArrayBuffer, options: ParseOptions = {}): Promise<ParseResult> {
    ...
    switch (version) {
      ...
      case 'GP5':
        warnings.push('GP5 format parsing is simplified - some features may not be preserved');
        // For now, throw error - GP5 parser would be implemented here
        throw new Error('GP5 format not yet fully implemented. Please use GP7/GP6 format for now.');
