...
mod tests {
    ...
    fn test_process_buffer_mismatch() {
        ...
        let mut output = vec![0.0f32; 512];  // Wrong size
        let result = engine.process(&input, &mut output, 2);
        assert!(result.is_err());
    }
    ...
    fn test_process_mono() {
        ...
        let result = engine.process(&input, &mut output, 1);
        assert!(result.is_ok());
    }

    #[test]
    fn test_eq_all_bands() {
        let mut engine = AudioEngine::new(48000, 512).unwrap();
        for band in 0..4 {
            let params = ffi::EQBand::new(1000.0, 0.0, 1.0);
            let result = engine.set_eq_band(band, params);
            assert!(result.is_ok());
        }
    }
    ...
    fn test_eq_enabled() {
        ...
        engine.set_eq_enabled(false);
    }

    #[test]
    fn test_compressor() {
        let mut engine = AudioEngine::new(48000, 512).unwrap();
        let params = ffi::CompressorParams::new(-20.0, 4.0, 10.0, 100.0, 2.0, 0.0);
        engine.set_compressor(params);
    }
    ...
    fn test_compressor_enabled() {
        ...
        engine.set_compressor_enabled(false);
    }

    #[test]
    fn test_reverb() {
        let mut engine = AudioEngine::new(48000, 512).unwrap();
        let params = ffi::ReverbParams::new(0.5, 1.5, 20.0, 0.3);
        engine.set_reverb(params);
    }
    ...
    fn test_delay() {
        let mut engine = AudioEngine::new(48000, 512).unwrap();
        let params = ffi::DelayParams::new(250.0, 0.4, 0.25);
        engine.set_delay(params);
    }

    #[test]
    fn test_delay_enabled() {
        let mut engine = AudioEngine::new(48000, 512).unwrap();
        engine.set_delay_enabled(true);
        engine.set_delay_enabled(false);
    }
    ...
    fn test_latency_calculation() {
        let engine = AudioEngine::new(48000, 512).unwrap();
        let samples = engine.get_latency_samples();
        let ms = engine.get_latency_ms();
        let expected_ms = (samples as f32 / 48000.0) * 1000.0;
        assert!((ms - expected_ms).abs() < 0.1);
    }
}
