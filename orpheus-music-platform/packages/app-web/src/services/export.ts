 ...
 * @deprecated Use exportProject('gp5') which provides proper warning handling
 * @todo Implement full GP5 binary format (complex file structure)
 */
export async function exportToGuitarPro(project: MaestroProject): Promise<Blob> {
  // NOTE: GP5 export not yet implemented. The exportProject() function
  // handles this gracefully by exporting MIDI with a user warning.
  // This function is kept for backwards compatibility.
  console.warn('[Export] Guitar Pro export not yet implemented - use exportProject("gp5") for proper warning handling');
...
export const EXPORT_FORMATS: ExportFormat[] = [
  ...
  },
  {
    ...
    name: 'Guitar Pro 5',
    extension: 'gp5',
    description: 'Coming soon - exports as MIDI for now',
...
/**
 * Export result with optional warning for fallback scenarios
 */
export interface ExportResult {
  blob: Blob;
  filename: string;
  warning?: string;
  actualFormat?: string; // The format that was actually exported (if different from requested)
}
...
export async function exportProject(
  ...
  switch (format) {
    ...
    case 'gp5':
      // Guitar Pro export not yet implemented - fall back to MIDI with clear warning
      blob = await exportToMIDI(project);
      extension = 'mid'; // Use correct extension, NOT .gp5
      actualFormat = 'midi';
      warning =
        'Guitar Pro 5 export is not yet available. Your project has been exported as MIDI instead, which can be imported into Guitar Pro or any DAW.';
      break;
  ...
  return { blob, filename, warning, actualFormat };
}
